...
class FeedbackResource (private val feedbackCreator: FeedbackCreator , private val feedbackFetcher: FeedbackFetcher, private val feedbackUpdater: FeedbackUpdater,private val request: HttpServletRequest) {
    ...
    fun updateStatus(@PathParam("id") feedbackId: Long,@PathParam("status") status: String): Response {
        val userLoggedInDetails = getUserDetails()
        return Response.ok(feedbackUpdater.updateStatus(userLoggedInDetails.companyId, enumValueOf(status.toString()), feedbackId)).build()
    }
